...
T: FightFromDistance + Debug,
{
        if distance < 10 {
            opponent.health -= 10;
            println!(
                "Your strike with your bow! Your opponent now has {} health left. You are now at : {:?}", 
...
fn attack_with_sword<T>(character: &T, opponent : &mut Monster)
    ...
    {
         opponent.health -= 10;
         println!("Your strike with your sword! Yout opponent's health is now {}. You are now at {:?}", 
         opponent.health, character
        );
    }
